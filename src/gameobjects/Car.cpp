...
namespace game
{
  Car::Car(uint64_t guid, const Config& config, const Scene& scene) :
    ...
    gun(config.physics.gun, scene.terrain, config.physics.car.connectionPoints.weapon.gun),
    cannon(config.physics.cannon, scene.terrain, config.physics.car.connectionPoints.weapon.cannon),
